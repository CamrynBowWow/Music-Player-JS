...
export async function setMusicValue(name, byteLength, type) {
	...
	try {
		await Promise.all([
			transactionMusic.store.put({ name: name, byteLength: byteLength, type: type }),
...
export async function storeMusicIntoPlaylists(playlistName, valueId) {
	...
	if (valueArray != null && valueId != null) {
		...
		valueArray.map((value) => {
			...
			} else if (value === valueId && playlistName === 'Favorites') {
				// Removes musicID from favorite if already in the database

				arrayValues = spliceMusic(arrayValues, valueId);
			} else if (value === valueId) {
				// If music already in playlist will send bool true back
...
export async function getNextId(musicId, playlistName, trueOrFalse, nextPrevValue, repeatValue) {
	...
	array.map(function (element, index, array) {
		if (parseInt(element) === parseInt(musicId)) {
			// Checks to see if values are the same then gets the next music id
...
function checkArrayValue(array, index, nextPrevValue, repeatValue) {
	...
	if (nextPrevValue === 'next' || repeatValue === 'repeatPlaylist') {
		// For the next song ID
		if (array[index] === array[array.length - 1]) {
			value = array[0]; // Will get the first song in the playlist to play
		} else {
			value = array[index + 1];
		}
	} else if (nextPrevValue === 'prev') {
		// For the previous song ID
